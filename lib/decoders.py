...
# from lib.PSA import PSAModule
# from lib.FFTFormer import FSAS
...
class MSCB(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, kernel_sizes=[1,3,5], expansion_factor=2, dw_parallel=True, add=True, activation='relu6'):
        ...
        # self.psa = PSAModule(self.ex_channels, self.ex_channels)
        if self.add == True:
    ...
    # def forward(self, x):
    #     pout1 = self.pconv1(x)
    #     psa_outs = self.psa(pout1)
    #     out = self.pconv2(psa_outs)
    #     if self.use_skip_connection:
    #         if self.in_channels != self.out_channels:
    #             x = self.conv1x1(x)
    #         return x + out
...
class EMCAD(nn.Module):
    def __init__(self, channels=[512,320,128,64], kernel_sizes=[1,3,5], expansion_factor=6, dw_parallel=True, add=True, lgag_ks=3, activation='relu6'):
        ...
        # # self.fsas1 = FSAS(channels[1], 14, 14)
        # # self.fsas2 = FSAS(channels[2], 28, 28)
        # # self.fsas3 = FSAS(channels[3], 56, 56)
        self.sab = SAB()
        # self.psa1 = PSAModule(channels[0],channels[0])
        # self.psa2 = PSAModule(channels[1],channels[1])
        # self.psa3 = PSAModule(channels[2],channels[2])
        # self.psa4 = PSAModule(channels[3], channels[3])
    ...
    def forward(self, x, skips):
        ...
        # EUCB3
        d3 = self.eucb3(d4)#[2, 320, 14, 14]
        # LGAG3
        x3 = self.lgag3(g=d3, x=skips[0])#[2, 320, 14, 14]
        # Additive aggregation 3
        d3 = d3 + x3
        # d3 = self.fsas1(d3,edge,edge)
